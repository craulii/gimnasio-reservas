...
export async function POST(request) {
  ...
  if (!creds)
    return new Response("Acceso no autorizado. Por favor, inicia sesión.", {
      status: 401,
    });
  ...
  if (!user)
    return new Response(
      "Acceso no autorizado. Usuario o contraseña incorrectos.",
      { status: 401 }
    );

  if (user.is_admin !== 0)
    return new Response("Solo los alumnos pueden realizar reservas.", {
      status: 403,
    });

  const { bloque_horario } = await request.json();
  if (!bloque_horario)
    return new Response("Debe seleccionar un bloque horario para reservar.", {
      status: 400,
    });
  ...
  try {
    ...
    if (reservados >= total) {
      return new Response("No hay cupos disponibles para este bloque", {
        status: 400,
      });
    }
    ...
    if (reservasHoy.length > 0) {
      const tieneEnBloque = reservasHoy.some(
        (r) => r.bloque_horario === bloque_horario
      );
      if (tieneEnBloque) {
        return new Response(
    ...
    return new Response(
      JSON.stringify({
        message: `Reserva realizada exitosamente para el bloque ${bloque_horario}. ¡Gracias!`,
      }),
