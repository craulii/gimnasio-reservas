...
// Asegúrate de que la ruta sea correcta. Si estás en components/admin/estadisticas, son 3 niveles hacia arriba.
...
export default function EstadisticasAlumno({ fechaInicio, fechaFin, setMessage, setLoading }) {
  ...
  const cargarEstadisticasAlumno = async () => {
    ...
    setLoading(true);
    try {
      // ✅ CORRECTO: Ya no pasamos 'user' ni headers manuales.
      // La cookie viaja sola gracias a credentials: 'include' en api.js
      ...
      if (ok) {
        ...
        // Manejo de error robusto compatible con el nuevo api.js
        const errorMsg = data?.error || data?.message || "Error cargando estadísticas del alumno";
        setMessage(String(errorMsg));
        setEstadisticasAlumno({}); // Limpiamos datos viejos si falla
      }
    } catch (error) {
      console.error("Error:", error);
      setMessage("Error de conexión al obtener estadísticas");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Preparar datos para gráficos (Protección contra null/undefined)
