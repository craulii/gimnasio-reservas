...
// Asegúrate de tener instalado: npm install exceljs file-saver
...
export default function ExportarDatos({ setMessage }) {
  ...
  const cargarMesesDisponibles = async () => {
    try {
      // ✅ Correcto: ApiService devuelve { ok, data }
  ...
  const exportarAExcel = async () => {
    ...
    try {
      ...
      // data contiene directamente el texto del CSV (o JSON), no necesitamos llamar a .text()
      const { ok, data } = await ApiService.exportarDatos(tipoExportacion, mesSeleccionado);

      if (ok && data) {
        // Aseguramos que tratamos 'data' como string para el parseo CSV
        const text = typeof data === 'object' ? JSON.stringify(data) : data;
        
        // Convertir CSV a array de objetos (Lógica original intacta)
        const lineas = text.split('\n').filter(l => l.trim());
        
        if (lineas.length === 0) {
            setMessage("No hay datos para exportar en este período.");
            setLoading(false);
            return;
        }
        ...
        // Usar file-saver para guardar (más compatible que crear URL object manual)
        saveAs(blob, fileName);
        
        setMessage(`✓ Archivo Excel descargado: ${fileName}`);
      } else {
        const errorMsg = data?.error || data?.message || "Error al exportar datos";
        setMessage(String(errorMsg));
      }
    } catch (error) {
      console.error("Error exportando:", error);
      setMessage("Error crítico al generar el Excel");
    } finally {
      setLoading(false);
    }
  };
