...
export default function EstadisticasBloque({ fechaInicio, fechaFin, cupos, setMessage, setLoading }) {
  ...
  const cargarEstadisticasBloque = async () => {
    ...
    setLoading(true);
    try {
      // La llamada es correcta, la cookie viaja sola gracias al nuevo api.js
      ...
      if (ok) {
        ...
        // MEJORA: Manejo de errores más descriptivo
        const errorMsg = data?.error || data?.message || "Error cargando estadísticas del bloque";
        setMessage(String(errorMsg));
        setEstadisticasBloque({}); // Limpiamos para evitar gráficos rotos
      }
    } catch (error) {
